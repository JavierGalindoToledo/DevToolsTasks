...
TEST(Javier_Galindo_Cell_Automaton, can_iterate_5_times) {
    std::vector<CellState> state{ CellState::DEAD, CellState::ALIVE, CellState::DEAD };
    CellularAutomaton automat(1, 3, state);
    ASSERT_NO_THROW(automat.iterate(5));
}

TEST(Javier_Galindo_Cell_Automaton, can_create_2_equal_auto) {
...
TEST(Javier_Galindo_Cell_Automaton, can_use_001_rule) {
...
TEST(Javier_Galindo_Cell_Automaton, can_use_010_rule) {
    std::vector<CellState> state{ CellState::DEAD, CellState::ALIVE, CellState::DEAD };
    CellularAutomaton automat(2, 3, state);
    automat.iterate(1);
    state = { CellState::DEAD, CellState::ALIVE, CellState::DEAD };
